    ...
    try {
      if (searhValue) {
        const response = await axios.get(
          `products/all?page=1&sortBy=createdAt&sortOrder=desc&branch_id=${branch_id}&category_id=${categoryId}&search=${searhValue}`,
        );
        setNearestBranchProduct(response.data.result.rows);
        setCount(response.data.result.count);
      } else {
        const response = await axios.get(
          `products/all?page=1&sortBy=createdAt&sortOrder=desc&branch_id=${branch_id}&category_id=${categoryId}&search=`,
        );
        setNearestBranchProduct(response.data.result.rows);
        setCount(response.data.result.count);
      }
            ...
            <section className="flex flex-col w-full">
              <div className="order-last lg:order-first mb-4 lg:mb-0 flex items-end tracking-tight gap-[0.7rem]">
                <div className="flex text-[24px]">
                  <h4 className="text-[#343538] font-semibold">
                    {nearestBranchProduct[0]?.Product?.Category?.name}
                  </h4>

...
export const createAdmin = async (req, res) => {
    try {
        ...
        if (findUser == null) {
            ...
            const data = fs.readFileSync(path.join(__dirname, '../templates/admin/emailVerification.html'), 'utf-8')
...
export const loginAdmin = async (req, res) => {
    try {
        ...
        let payload = { id: dataLoginAdmin.id, isSuperAdmin: dataLoginAdmin.isSuperAdmin };
...
export const forgotPassword = async (req, res) => {
    try {
        ...
        if (findAccount) {
            ...
            const data = fs.readFileSync(path.join(__dirname, '../templates/admin/forgotPassword.html'), 'utf-8')
            const tempCompile = await handlebars.compile(data);
            const tempResult = tempCompile({
                email: email,
...
export const getTotalAdmin = async (req, res) => {
    try {
        ...
        res.status(200).send({ totalAdmin })
    } catch (error) {
        console.error(error)
        return res.status(500).send({ message: error.message })
    }
